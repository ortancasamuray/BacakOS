...
| `userdirs`    | ✅ done   | First-run localisation of the standard XDG user directories to Turkish names — renames/creates the Turkish-named folder and rewrites `~/.config/user-dirs.dirs` so the sidebar follows; guarded by a marker so it only runs once and never overwrites an existing Turkish directory. |
